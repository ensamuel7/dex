...
func init() {
	Register(&Module{
		...
		Funcs: map[string]FuncDef{
			...
			"format": {
				Params:     []ast.Type{ast.TypeLong, ast.TypeString},
				ParamNames: []string{"timestamp", "layout"},
				ReturnType: ast.TypeString,
				CName:      "dex_time_format",
				Doc:        "Format a Unix timestamp (seconds) using a strftime layout or \"iso8601\".",
			},
			"isoNow": {
				Params:     []ast.Type{},
				ReturnType: ast.TypeString,
				CName:      "dex_time_iso_now",
				Doc:        "Return the current time as an ISO 8601 string (UTC).",
			},
			"unixNow": {
				Params:     []ast.Type{},
				ReturnType: ast.TypeLong,
				CName:      "dex_time_unix_now",
				Doc:        "Return the current Unix timestamp in seconds (wall clock).",
			},
		},
